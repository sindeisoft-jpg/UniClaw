...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">不支持的配置节点，请使用原始模式。</div>
    </div>`;
  }
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">不支持的数组结构，请使用原始模式。</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="cfg-array">
      ...
      ${
        arr.length === 0
          ? html`
              <div class="cfg-array__empty">暂无项，点击「添加」创建。</div>
            `
          : html`
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">自定义项</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
      ...
      ${
        entries.length === 0
          ? html`
              <div class="cfg-map__empty">暂无自定义项。</div>
            `
          : html`
