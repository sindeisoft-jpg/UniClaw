...
const TOOL_SECTIONS = [
  ...
  },
  {
    ...
    tools: [
      ...
      { id: "sessions_spawn", label: "sessions_spawn", description: "创建子智能体" },
  ...
  },
  {
    ...
    label: "自动化",
    tools: [
      { id: "cron", label: "cron", description: "安排任务" },
      ...
      <section class="agents-main">
        ${
          !selectedAgent
            ? html`
                <div class="card">
                  <div class="card-title">选择智能体</div>
                  <div class="card-sub">选择智能体以查看其工作区与工具。</div>
                </div>
              `
            : html`
              ${renderAgentHeader(
...
function renderAgentHeader(
  ...
  const subtitle = agent.identity?.theme?.trim() || "智能体工作区与路由。";
...
function renderAgentContextCard(context: AgentContext, subtitle: string) {
  return html`
    <section class="card">
      <div class="card-title">智能体上下文</div>
      <div class="card-sub">${subtitle}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">工作区</div>
...
function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, "工作区、身份与模型配置。")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">渠道</div>
            <div class="card-sub">网关级渠道状态快照。</div>
          </div>
...
function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(context, "工作区与调度目标。")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
    ...
    <section class="card">
      <div class="card-title">智能体定时任务</div>
      <div class="card-sub">以本智能体为目标的定时任务。</div>
      ${
        jobs.length === 0
          ? html`
    ...
    <section class="card">
      ...
      ${
        hasGlobalAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                已设置全局 tools.allow，智能体覆盖无法启用被全局禁止的工具。
              </div>
            `
          : nothing
      }
  ...
  return html`
    <section class="card">
      ...
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">本智能体使用自定义技能允许列表。</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
                全部技能已启用，禁用任意技能将创建按智能体的允许列表。
              </div>
            `
      }
      ${
