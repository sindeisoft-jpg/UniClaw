...
import { UI_HINTS_ZH } from "../config-i18n-zh.ts";
...
export function applyConfigSchema(state: ConfigState, res: ConfigSchemaResponse) {
  ...
  const merged: ConfigUiHints = { ...serverHints };
  for (const [path, zh] of Object.entries(UI_HINTS_ZH)) {
    merged[path] = { ...serverHints[path], ...zh };
  }
  state.configUiHints = merged;
