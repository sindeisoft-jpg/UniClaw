...
import { clampNavWidth } from "./storage.ts";

function handleNavResizeStart(state: AppViewState, e: MouseEvent): void {
  if (e.button !== 0) {
    return;
  }
  e.preventDefault();
  ...
  const onMove = (e2: MouseEvent): void => {
    ...
    const newWidth = clampNavWidth(startWidth + delta);
    state.applySettings({ ...state.settings, navWidth: newWidth });
  };
  const onUp = (): void => {
    document.removeEventListener("mousemove", onMove);
    document.removeEventListener("mouseup", onUp);
    document.body.classList.remove("shell-resize-dragging");
  };

  document.body.classList.add("shell-resize-dragging");
        ...
        <div class="topbar-left">
          ...
          <div class="brand">
            <div class="brand-text">
              <div class="brand-title">紫鈊一号</div>
        ...
        !state.settings.navCollapsed
          ? html`
              <div
                ...
                aria-label="拖拽调整侧栏宽度"
                title="拖拽调整侧栏宽度"
                @mousedown=${(e: MouseEvent) => handleNavResizeStart(state, e)}
              ></div>
            `
          : nothing
      }
