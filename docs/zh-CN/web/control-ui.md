...
控制 UI 是一个由 Gateway 网关提供服务的 **Next.js + React + Tailwind**（V0 技术栈）应用：
...
控制 UI 是静态文件；WebSocket 目标是可配置的，可以与 HTTP 源不同。当你想要在本地使用 Next.js 开发服务器但 Gateway 网关在其他地方运行时，这很方便。

1. 启动 UI 开发服务器：`pnpm ui:dev`
2. 打开类似以下的 URL：

```text
http://localhost:3000/?gatewayUrl=ws://<gateway-host>:18789
```

可选的一次性认证（如需要）：

```text
http://localhost:3000/?gatewayUrl=wss://<gateway-host>:18789&token=<gateway-token>
```
