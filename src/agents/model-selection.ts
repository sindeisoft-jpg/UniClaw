...
export function buildAllowedModelSet(params: {
  ...
  /** Providers that appear in the allowlist; any model from these providers is allowed (e.g. live Ollama models not in static catalog). */
  providersInAllowlist: Set<string>;
} {
  const rawAllowlist = (() => {
  ...
  for (const raw of rawAllowlist) {
    ...
    providersInAllowlist.add(providerKey);
  ...
  // Align with models.list (configuredOnly): any catalog model whose provider
  // appears in the allowlist is allowed, not only exact keys.
  for (const entry of params.catalog) {
    if (providersInAllowlist.has(normalizeProviderId(entry.provider))) {
