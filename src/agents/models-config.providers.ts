...
export async function resolveImplicitProviders(params: {
  ...
  // Ollama provider: add when user has OLLAMA_API_KEY/auth, or always add local default (127.0.0.1:11434)
  // so the local Ollama service is available without any config.
  const ollamaKey =
  ...
  } else if (!params.explicitProviders?.ollama) {
    // No explicit ollama config: add local default 127.0.0.1:11434 so it appears in model list.
    providers.ollama = {
      ...(await buildOllamaProvider(undefined)),
      apiKey: "ollama",
    };
  }
