...
describe("Ollama provider", () => {
  it("should include local ollama (127.0.0.1:11434) when no API key and no explicit config", async () => {
    const agentDir = mkdtempSync(join(tmpdir(), "openclaw-test-"));
    const providers = await resolveImplicitProviders({ agentDir });

    // Local Ollama at 127.0.0.1:11434 is always added so it appears in the model list.
    expect(providers?.ollama).toBeDefined();
    expect(providers?.ollama?.apiKey).toBe("ollama");
    expect(providers?.ollama?.baseUrl).toMatch(/127\.0\.0\.1:11434\/v1/);
  });
