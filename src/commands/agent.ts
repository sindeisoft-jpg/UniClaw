    ...
    try {
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: (providerOverride, modelOverride) => {
          ...
          const modelKeyForToolChoice = modelKey(providerOverride, modelOverride);
          const effectiveToolChoice =
            cfg.agents?.defaults?.models?.[modelKeyForToolChoice]?.toolChoice ??
            cfg.agents?.defaults?.toolChoice ??
            "auto";
          const extraPrompt =
            effectiveToolChoice === "required"
              ? `${opts.extraSystemPrompt ?? ""}\n\nYou must call one of the available tools before responding.`.trim()
              : opts.extraSystemPrompt;
          return runEmbeddedPiAgent({
            ...
            disableTools: cfg.agents?.defaults?.enableTools === false,
            provider: providerOverride,
            model: modelOverride,
            authProfileId,
