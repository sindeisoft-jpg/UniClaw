...
describe("control UI assets helpers", () => {
  it("resolves repo root from src argv1", async () => {
    ...
    try {
      // 创建完整的目录结构，包括 apps/control-ui/next.config.ts
      await fs.mkdir(path.join(tmp, "apps", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "apps", "control-ui", "next.config.ts"), "export {};\n");
