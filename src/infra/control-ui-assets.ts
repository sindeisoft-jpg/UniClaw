...
export function resolveControlUiRepoRoot(
  ...
  if (srcIndex !== -1) {
    ...
    if (fs.existsSync(path.join(root, "apps", "control-ui", "next.config.ts")) ||
        fs.existsSync(path.join(root, "apps", "control-ui", "next.config.js"))) {
      return root;
    }
  }
  ...
  for (let i = 0; i < 8; i++) {
    if (
      ...
      (fs.existsSync(path.join(dir, "apps", "control-ui", "next.config.ts")) ||
        fs.existsSync(path.join(dir, "apps", "control-ui", "next.config.js")))
    ) {
      return dir;
    }
