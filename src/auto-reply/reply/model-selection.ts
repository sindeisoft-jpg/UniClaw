...
export async function createModelSelectionState(params: {
  ...
  let providersInAllowlist = new Set<string>();
  ...
  if (needsModelCatalog) {
    ...
    providersInAllowlist = allowed.providersInAllowlist ?? new Set<string>();
  }

  // Don't reset session override when the model's provider is in the allowlist (e.g. ollama);
  // the static catalog may not include live models (e.g. ollama/qwen3), but UI allows them.
  if (sessionEntry && sessionStore && sessionKey && hasStoredOverride) {
    ...
    if (overrideModel) {
      ...
      const providerAllowed =
        providersInAllowlist.has(normalizeProviderId(overrideProvider));
      if (
  ...
  if (storedOverride?.model && !skipStoredOverride) {
    ...
    const providerAllowed =
      providersInAllowlist.has(normalizeProviderId(candidateProvider));
    if (
      allowedModelKeys.size === 0 ||
      allowedModelKeys.has(key) ||
      providerAllowed
